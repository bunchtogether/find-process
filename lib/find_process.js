...
const finders = {
  ...
  win32 (cond) {
    return new Promise((resolve, reject) => {
      const lines = []
      const proc = utils.spawn('wmic', ['path', 'win32_process', 'get', 'Name,Processid,ParentProcessId,Commandline,ExecutablePath'], { detached: false, windowsHide: true })
      proc.stdout.on('data', data => {
        lines.push(data.toString())
      })
      proc.on('close', code => {
        if (code !== 0) {
          return reject(new Error('Command terminated with code: ' + code))
        }
